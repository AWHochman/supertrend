...
class SuperTrendStrategy(bt.SignalStrategy):
    ...
    def next(self):
        ...
        if (self.data.close > self.st) and (self.ema3 < self.ema20) and (self.status != 1) and self.obv[0] > self.obv[-1]:
            numShares = (self.broker.getvalue() -
                         self.broker.getcash())/self.data.close
            numToSell = int(numShares*self.exchange_amt)
            self.sell(data=self.data, size=numToSell)
            self.status = 1

        if (self.data.close < self.st) and (self.ema3 > self.ema20) and (self.status != 2) and self.obv[0] > self.obv[-1]:
            numSharesToBuy = int(self.broker.getcash() /
                                 self.data.close) * self.exchange_amt
...
def main():
    ...
    ma = bt.feeds.PandasData(
        dataname=yf.download('AAPL', datetime(2019, 1, 1), datetime(2021, 1, 1)))
